...
describe("Cost Element Detail AI Assessment Tab", () => {
  afterEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("renders AI Assessment tab in tabs list", async () => {
    vi.spyOn(Route, "useParams").mockReturnValue({
      id: "project-123",
      wbeId: "wbe-123",
      costElementId: "cost-element-123",
    } as any)
    vi.spyOn(Route, "useSearch").mockReturnValue({
      view: "cost-registrations",
    } as any)
    ...
    vi.mocked(client.CostElementsService.readCostElement).mockResolvedValue({
      cost_element_id: "cost-element-123",
  ...
  it("renders AIChat component when AI Assessment tab is selected", async () => {
    ...
    await waitFor(() => {
      ...
      expect(aiChat).toHaveAttribute("data-context-type", "cost-element")
      expect(aiChat).toHaveAttribute("data-context-id", "cost-element-123")
    })
  })
})
